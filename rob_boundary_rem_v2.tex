...
Let us consider a bounded open set $\Omega\subset \mathbb{R}^N$, $N\geq 1$ with boundary $\partial \Omega$ of class $\mathcal C^2$. Let $\omega\subset \Omega$ be a nonempty open subset and \blue{$\cbd$ be a nonempty open subset of $\partial \Omega$}.  Given $T>0$, we will use the notation $Q:=\Omega\times(0,T)$ and $\Sigma:=\partial \Omega\times(0,T)$, while $n(x)$ will denote the outward unit normal vector at the point $x\in \partial \Omega$. 
...
\end{equation}
%
where $y_0$ is a given initial datum and $\psi$ is an unknown perturbation.
...
\blue{A partir de aqui hay un cambio gordo, no me gusta la notacion de $\mathbf{1}$ para referirme a las indicadores, prefiero $\chi$. Por esta razon adopte la notacion $\chi$ para las indicadoras, y la funcion que aproxima a una indicadora la denote como $\rho$. Como es de esperarse, el subindice esta asociado al conjunto sobre el que se aplica, por ejemplo $\chi_\omega$ es la indicadora de $\omega$ o $\rho_\Gamma$ es la approximacion suave de la indicadora en $\Gamma$. }
...
In a first result, we address the robust hierarchic control of system \eqref{heat_lin}, that is, the case where the follower control is applied on the boundary and the leader control is localized on the interior of the domain. In this regard, we have the following
%
...
\end{equation*}
%
we can find a leader control $h\in L^2(\Gamma\times(0,T))$ and a unique solution $(\bar v,\bar\psi)\in L^2(\mathcal B_1\times(0,T))\times L^2(\mathcal B_2\times(0,T))$ to the robust control problem for \eqref{func_rob_dif}, such that the associated solution to \eqref{heat_dif} verifies $y(\cdot,T)=0$ in $\Omega$. 
%
\end{theorem}
...
w=h\chi_{\cbd_1}+ v{\csbd}_{2} &\text{on } \Sigma, \\
...
Multiplying \eqref{adj_frontera} by $y_v$ in $L^2(Q)$ and integrating by parts, we obtain  \magenta{de donde sali\'o esa $x_{O}$?} \blue{VH: Sale de integrar multiplicar 2.7 por $y^\prime$ solucion de (2.4 ) e integrar por partes.}
%
\begin{equation*}
...
\blue{Observe that the first statement implies Theorem \ref{teo_main1}. The equivalence between the statements of Proposition \ref{prop_control} is by now standard and relies on several classical arguments, so we omit the proof. The main idea follows from the well-known Hilbert uniqueness method (HUM for short), see, e.g., \cite{glo_lions_he,boyer_HUM}. For each $\varepsilon>0$, we consider the functional
%
\begin{equation}\label{func_h_leader}
F_\varepsilon(\varphi^T)=\frac{1}{2}\iint_{\omega\times(0,T)}|\varphi|^2\dx\dt+\frac{\varepsilon}{2}\|\varphi^T\|_{H_0^1(\Omega)}+\langle y_0,\varphi(0)\rangle_{-1,1}-\iint_{Q}\theta y_d\dx\dt, \quad \varepsilon>0,
\end{equation}
%
where $\langle\cdot,\cdot\rangle_{-1,1}$ denotes the duality product between $H^{-1}(\Omega)$ and $H_0^1(\Omega)$. One can prove that \eqref{func_h_leader} is continuous and strictly convex. Moreover, thanks to \eqref{obs_ineq_1} we can prove that $F_\varepsilon$ is coercive and thus has a unique minimizer that we denote as $\varphi_\varepsilon^T$. In this way, we can construct a sequence of controls $h_\varepsilon=\varphi_\varepsilon|_{\omega}$, where $\varphi_\varepsilon$ is the associated solution to \eqref{adj_sys_foll} with initial datum $\varphi_\varepsilon^T$, such that the controlled system \eqref{sys_foll} verifies $\|y(T)\|_{H^{-1}(\Omega)}\leq \varepsilon$. This fact, together with some uniform estimates on $h_\varepsilon$, enable us to obtain a null control as a limit process as $\varepsilon\to 0$. For the interested reader, we refer to \cite{araruna,vhs_deT_rob} where a detailed exposition is addressed for similar hierarchic control problems. }
...
%
\begin{remark}
In \cite{fursi,cara_guerrero}, the authors use the function $l(t)=t(T-t)$ to prove the above lemma. Adapting the result with the weights in \eqref{weights_l} does not change the result, since the important property is that $l(t)$ goes to 0 as $t$ tends to 0 a $T$.
\end{remark}
...
%
\begin{proposition}\label{prop_carleman}\magenta{checar esta parte con m\'as cuidado en la redacci\'on de las pruebas porque cambia dr\'asticamente con la secci\'on anterior y la posteriort} \blue{VH: Checado, creo qeu todo esta correcto}\\
...
The observability inequality \eqref{obs_ineq_1} follows the Carleman estimate \eqref{car_final}, some weighted energy estimates and it is based on well-known arguments which can be adapted from \cite[Lemma 1]{cara_NS}. For the sake of completeness, we sketch it briefly. 
...
\end{equation}
%
Finally, we obtain the observability inequality \eqref{obs_ineq_1} with $\vartheta(t)=e^{s\beta^\star}$ by combining estimates \eqref{obs_mu}--\eqref{est_C_mu} and using the definition of $\beta^\star$ and $\phi^\star$ and taking $\gamma,\ell>>1$. This concludes the proof. 
...
It is well-known (see, e.g., \cite{ima_original,cara_guerrero}) that for $v\equiv\psi\equiv 0$, system \eqref{heat_dif1} is null controllable at time $T$ for any open set $\mathcal O\subset\partial \Omega$ \magenta{?`no es el mismo que el anterior?} \blue{VH: No, observa que cuando pones $v=\psi=0$ en (3.1) el sistema que queda tiene control $h$ en la frontera. En el caso anterior, cuando pones $v=\psi=0$ te queda el $h$ la ecuacion. Aunque no parece mucho, el cambio es delicado}. Here, we will see that using a hierarchic methodology for solving first the associated robust control problem introduces additional difficulties when dealing with the null controllability objective. 
...
z=h\chi_{\cbd}, \quad p=0 &\text{on } \Sigma, \\
...
As mentioned in the previous section \magenta{again ?`d\'onde se mencion\'o? } \blue{VH: ve el cambio en azul que introduje en la seccion anterior pp.8}, we can build the control $h$ of minimal norm by adapting the well-known HUM method. For this particular case, the main ingredient lies at finding, for each $\varepsilon>0$, the minimizers of the functional
%
...
To conclude, it is enough to combine estimates \eqref{aux7} and \eqref{est_local_fin} and take the parameters $\ell,\gamma$ large enough to absorb the remaining terms. Then, \eqref{obs_ineq_dif} follows from the resulting expression by setting $\vartheta_2(t):=e^{s\tilde\beta_2^\star}$, where we denote $\tilde{\beta}_2^\star(t)=\max_{x\in\overline{\Omega}}\tilde\beta_2(x,t)$, and recalling that $\mathcal S^\prime\subset\subset \mathcal O$.  This ends the proof. 
%
\end{proof}
...
w=h\chi_{\mathcal O_1}+ \frac{1}{\ell^2}\frac{\partial r}{\partial n}\rho_\star^{-2}{\csbd}_2, \quad r=0 &\quad \text{on } \Sigma, \\
...
&\leq C\iint_{\mathcal \mathcal O_1\times(0,T)}e^{-2s\bar\alpha}s\bar\xi\left|\frac{\partial \varphi}{\partial n}\right|^2\d\sigma\dt+C\iint_{Q}e^{-2s\bar\alpha^\star}(s\bar\xi^\star)^3|\theta|^2\dx\dt+\frac{C}{\ell^4}\|e^{-s\bar\alpha^\star}\rho_\star^{-2}\varphi\|^2_{H^{2,1}(Q)}
\end{align}
%
...
\begin{cases}
w_t-\Delta w=0, & \quad \text{in Q}, \\
w=h\chi_{\cbd}+ v^1\rho_{\mathcal O_1}+v^2\rho_{\mathcal O_2}&\quad \text{on } \Sigma, \\
w(x,0)=w^0(x), &\quad \text{in } \Omega,
\end{cases}
\end{equation}
% 
...
w=h\chi_{\cbd}+ \frac{1}{\ell_1^2}\frac{\partial r^{1}}{\partial n}\rho_\star^{-2}\rho_{\mathcal O_1}+\frac{1}{\ell_2^2}\frac{\partial r^{2}}{\partial n}\rho_\star^{-2}\rho_{\mathcal O_2}, \quad r^{i}=0 &\quad \text{on } \Sigma, \\
...
UNAM (Mexico). Also, the author wish to acknowledge  Prof. Luz de Teresa (UNAM, Mexico) and Prof. Franck Boyer (IMT, France) for the fruitful discussions. 
\end{enumerate}  
