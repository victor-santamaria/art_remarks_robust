...
%
Optimization and control problems arise in many applications of engineering and mathematics. Traditionally, such problems deal with a single objective: minimize cost, maximize benefit, etc. \magenta{Aqu\'i hay que agregar referencias} However, when studying more realistic and complex situations, it is desirable to include several different objectives and therefore the introduction of multi-objective optimization is essential. 
...
In a first result, we address the robust hierarchic control of system \eqref{heat_lin}, that is, the case where the follower control is applied on the boundary and the leader control is \magenta{ a distributed one. esto que significa?} In this regard, we have the following
%
...
\end{equation}
%
we can find a leader control $h\in L^2(\omega\times(0,T))$ and a unique solution $(\bar v,\bar\psi)\in L^2(\Sigma)\times L^2(Q)$, to the robust control problem \eqref{func_rob}, such that the associated solution of \eqref{heat_lin} verifies $y(\cdot,T)=0$ in $\Omega$. \magenta{ corrigelo bien}
%
\end{theorem}

As usual in robust control problems, the assumption on $\gamma$ means that the possible disturbances \magenta{spoiling the control objectives should have moderate $L^2$-norms. No entiendo} Indeed, without this condition, it is not possible to prove the existence of the saddle point. On the other hand, the \magenta{assumption ?`cu\'al?} on the target $y_d$ means that should approach 0 as $t\to T$. This is a standard feature in some null controllability problems and has been discussed, for instance, in \cite{araruna,deteresa2000}. 
...
\end{equation}
%
As before, we will see that the robust control can be solved by selecting appropriate parameters $\ell$ and $\gamma$. Notice that unlike \eqref{heat_dif}, here we maximize for disturbances $\psi$ localized in the region $\mathcal B_2$. This comes from a technical reason concerning the resolution of the null controllability objective (see Section \magenta{\ref{}} for details), but which is not necessary to solve the robust control part. 
...
\end{equation}
%
\magenta{ lo mismo que en el anterior}
we can find a leader control $h\in \magenta{X ?`por que en magenta?}$ and a unique saddle point $(\bar v,\bar\psi)\in L^2(\mathcal B_1\times(0,T))\times L^2(\mathcal B_2\times(0,T))$, for the functional given by \eqref{func_rob_dif}, such that the associated solution to \eqref{heat_dif} verifies $y(\cdot,T)=0$ in $\Omega$. 
%
\end{theorem}

Hypothesis \eqref{loc_teo2} plays a fundamental role in the selection of the weight functions participating in the Carleman estimates needed to prove Theorem \ref{teo_main2}. This is not a common selection \magenta{falta decir algo m\'as creo} and the proof can only be achieved thanks to the special structure of the adjoint system. This particular selection has been recently used in other hierarchic control problems, see \cite{da_silva}. 
...
\end{equation}
%
We provide a partial answer to this problem for the case where $\psi\equiv 0$ and the cost functional corresponding to the optimization \magenta{problem takes the form ?`no se lee raro?}
%
...
\end{equation*}
%
where $y^\prime$ is solution to \eqref{deriv_sys1} with $v^\prime=0$. Since $y^\prime$ is clearly independent of $\tau$, a further computations yield
%
\begin{equation*}
...
Multiplying \eqref{adj_frontera} by $y_v$ in $L^2(Q)$ and integrating by parts, we obtain  \magenta{de donde sali\'o esa $x_{O}$?}
%
\begin{equation*}
...
%
\begin{proposition}\label{prop_carleman}\magenta{checar esta parte con m\'as cuidado en la redacci\'on de las pruebas porque cambia dr\'asticamente con la secci\'on anterior y la posteriort}\\
...
\begin{equation*}
\begin{split}
I(s;\varphi)+\widetilde{I}(s;\theta)\leq C&\left(\iint_{\omega_0\times(0,T)}e^{-2s\alpha}\left(s^3\xi^3|\varphi|^2+s\xi|\theta|^2\right)\dx\dt +\iint_Q e^{-2s\alpha}\left(|\theta\mathbf{1}_{\mathcal O_d}|^2+s^{-2}\xi^{-2}|\tfrac{1}{\gamma^2}\varphi|^2\right)\dx\dt\right. \\
...
\begin{split}
I(s;\varphi)+\widetilde{I}(s;\theta)\leq C&\left(\iint_{\omega_0\times(0,T)}e^{-2s\alpha}\left(s^3\xi^3|\varphi|^2+s\xi|\theta|^2\right)\dx\dt + s^{-\frac{1}{2}}\left\| e^{-2s\alpha}\xi^{-\frac{1}{4}+\frac{1}{m}} \frac{1}{\ell^2}\frac{\partial \varphi}{\partial \nu} \csbd \right\|^{2}_{L^2(\Sigma)} \right. \\
...
Then, we proceed to estimate the first boundary term in \eqref{car_con_bound}. Applying the same ideas as in \cite{duprez_lissy}, we take a function $\kappa\in C^2(\overline \Omega)$ such that
%
\begin{equation*}
...
\begin{split}
\iint_{\Sigma}e^{-2s\alpha}\left|\frac{\partial \varphi}{\partial \nu}\right|^2\d\sigma \dt&\leq C\left(\iint_Q e^{-2s\alpha^*}\left((s\xi)^{-1}|\Delta \varphi|^2 + s\xi|\nabla \varphi|^2\right)\dx\dt+\int_{0}^{T}e^{-2s\alpha^*}(s\xi^*)^{-1}\|\varphi\|^2_{H^2(\Omega)}\dt\right).
\end{split}
\end{equation}
...
\int_{0}^T e^{-2s\alpha^*}(s\xi^{*})^{-1}\|\varphi\|_{H^2(\Omega)}^2\dt\leq C\left(\iint_Qe^{-2s\alpha^*}(s\xi^*)^{-1}|\theta|^2\dx\dt+\iint_Qe^{-2s\alpha^*}(s\xi^*)^{-1+2(m+1)/m}|\varphi|^2\dx\dt\right)
\end{equation}
...
\begin{equation*}
\begin{split}
\iint_{\omega_0\times(0,T)}e^{-2s\alpha}s\xi|\theta|^2\dx\dt\leq& \frac{1}{\gamma^2}\iint_{Q}e^{-2s\alpha}s\xi |\varphi|^2\dx\dt+\iint_{\omega_1\times(0,T)}e^{-2s\alpha}\left(s^3\xi^3|\theta| |\varphi|+s^2\xi^2|\nabla \theta||\varphi|\right)\dx\dt,
\end{split}
\end{equation*}
%
...
\begin{split}
\iint_{\omega_0\times(0,T)}e^{-2s\alpha}s\xi|\theta|^2\dx\dt\leq& \frac{1}{\gamma^2}\iint_{Q}e^{-2s\alpha}s\xi |\varphi|^2+\varepsilon \widetilde I(s;\theta)\dx\dt+s^5\iint_{\omega_1\times(0,T)}e^{-2s\alpha}\xi^5|\varphi|^2\dx\dt.
\end{split}
\end{equation}
%
...
%
\begin{equation*}
-\frac{1}{2}\frac{\d}{\dt}\int_{\Omega}|\nabla\varphi|^2\tilde\eta\dx+\int_{\Omega}|\Delta\varphi|^2\tilde\eta\dx\leq \delta \int_{\Omega}|\Delta\varphi|^2\tilde\eta\dx+C(\delta)\int_{\Omega}|\theta|^2\tilde\eta\dx+\frac{1}{2}\int_{\Omega}|\nabla\varphi|^2|\eta^\prime|\dx
\end{equation*}
%
...
It is well-known (see, e.g., \cite{ima_original,cara_guerrero}) that for $v\equiv\psi\equiv 0$, system \eqref{heat_dif1} is null controllable at time $T$ for any open set $\mathcal O\subset\partial \Omega$ \magenta{?`no es el mismo que el anterior?}. Here, we will see that using a hierarchic methodology for solving first the associated robust control problem introduces additional difficulties when dealing with the null controllability objective. 
...
Using once \magenta{again ?`d\'onde se mencion\'o? }the Fenchel-Rockafellar technique, we can build the control $h$ of minimal norm through the minimizers of the dual functional
%
...
\end{split}\end{equation*}
 %
One can prove that the following observability inequality holds:
 %
\begin{equation}
\label{aux7}
\begin{split}
...
%
Assume that $\ell$ is large enough. There exists a constant $\lambda_0$ such that for any $\lambda\geq \lambda_0$ and a constant $C>0$ such that the solution $(\varphi,\theta)$ to \eqref{adj_sys_3} satisfies 
%
