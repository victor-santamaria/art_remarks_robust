...
w= h{\csbd}_{1}+ v{\csbd}_{2} &\text{on } \Sigma, \\
...
Thanks to Lemma \ref{prof_regularity} and using the above estimates, we can prove a Carleman inequality for the solution of the adjoint system \eqref{adj_sys_foll}. This will be the main ingredient to prove the observability inequality \eqref{obs_ineq_1}. The result is the following.
%
...
\end{equation}
%
Finally, we obtain the observability inequality \eqref{obs_ineq_1} with $\vartheta(t)=e^{s\beta^\star}$ by combining estimates \eqref{obs_mu}--\eqref{est_C_mu}, using the definition of $\beta^\star$ and $\phi^\star$ and taking $\gamma,\ell>>1$. This concludes the proof. 
