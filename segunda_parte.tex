...
\begin{equation*}
\begin{split}
\tilde{\xi}_i=&\frac{e^{\lambda\eta_i(x)}}{t^2(T-t)^2},\qquad \alpha_i=\frac{e^{\lambda(\|\eta_1\|+\|\eta_2\|)}-e^{\lambda \eta_i(x)}}{t^2(T-t)^2}.
\end{split}\end{equation*}
\end{notation}
%
\begin{remark}
...
\magenta{for $\lambda>1$. Hubo un procedimiento para llegar al siguiente estimado usando el anterior. No tengo le procedimiento y no lo repet\'i, adem\'as no s\'e que pas\'o con la $\lambda^2$} 
 ...
 \begin{split}
\iint_{Q}e^{-2s\tilde{\alpha}}(s\tilde{\xi})^{1+m}|\nabla q|^2\dx\dt+&\iint_{Q}e^{-2s\tilde{\alpha}}(s\tilde{\xi})^{3+m}|q|^2\\
&\leq C\left(\iint_{Q}e^{-2s\tilde{\alpha}}(s\tilde{\xi})^m|f|^2\dx\dt+\iint_{\Sigma^{\prime}}e^{-2s\tilde{\alpha}}(s\tilde{\xi})^{1+m}\left|\frac{\partial q}{\partial n}\right|^2\dx\dt\right),
\end{split}
\end{equation}
%
for $m\in$.
...
\begin{align}\notag
&I(\varphi)+I(\theta)\\\notag
&\leq C\left(\iint_{Q}e^{-2s\tilde{\alpha_2}}(s\tilde{\xi}_2)^m|\theta|^2\dx\dt+\iint_{\Sigma^{\prime}}e^{-2s\tilde{\alpha_1}}(s\tilde{\xi}_1)^{1+m}\left|\frac{\partial \varphi}{\partial n}\right|^2\dx\dt\right.\\
&\quad+\left.\iint_{Q}e^{-2s\tilde{\alpha_1}}(s\tilde{\xi}_1)^m|-\frac{1}{\ell^2}\varphi\csin{\mathcal B_1}+\frac{1}{\gamma^2}\varphi|^2\dx\dt+\iint_{\Sigma^{\prime}}e^{-2s\tilde{\alpha_1}}(s\tilde{\xi}_1)^{1+m}\left|\frac{\partial \theta}{\partial n}\right|^2\dx\dt\right)\\
&\leq C\left(\iint_{\Sigma^{\prime}}e^{-2s\tilde{\alpha_1}}(s\tilde{\xi}_1)^{1+m}\left|\frac{\partial \varphi}{\partial n}\right|^2\dx\dt+\iint_{\Sigma^{\prime}}e^{-2s\tilde{\alpha_2}}(s\tilde{\xi}_2)^{1+m}\left|\frac{\partial \theta}{\partial n}\right|^2\dx\dt\right).
\end{align}
%
where
\begin{equation*}
\begin{split}
I(\varphi)&:=\iint_{Q}e^{-2s\tilde{\alpha_1}}(s\tilde{\xi}_1)^{1+m}|\nabla \varphi|^2\dx\dt+\iint_{Q}e^{-2s\tilde{\alpha_1}}(s\tilde{\xi}_1)^{3+m}|\varphi|^2\dx\dt,\\
I(\theta)&:=\iint_{Q}e^{-2s\tilde{\alpha_2}}(s\tilde{\xi}_2)^{1+m}|\nabla \theta|^2\dx\dt+\iint_{Q}e^{-2s\tilde{\alpha_2}}(s\tilde{\xi}_2)^{3+m}|\theta|^2\dx\dt
\end{split}
\end{equation*}
