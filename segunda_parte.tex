...
\begin{notation}\label{notation3.1}
\magenta{creo que no es notaci\'on. Por otro lado, no s\'e quien es $S^{'}$, creo que $S^{\prime}\subset\subset \mathcal{O}$ and por otro lado en alg\'un lado se pone que $\bar{\mathcal O}_d\cap \bar{\mathcal B_i}=\emptyset$}\\
Consider the weight functions $\eta_1,\eta_2 \in C^2(\bar{\Omega})$ such that

\begin{equation}
\eta_1\geq \eta_2 \text{  on  }\Omega, \quad\eta_1=\eta_2\text{  on  }\mathcal O_d,\quad\eta_1\geq \max_{x\in \bar{\mathcal B}_i} \eta_2 \quad i=1,2.
\end{equation}
...
%
and $|\bar{\eta}^{\prime}|\leq C/T$ for some constant $C$. \end{notation}
%
\begin{remark}
We can note that,
\begin{equation*}
...
\begin{proposition}
\magenta{Under the hypotheses .... $\Sigma^{\prime}=S^{\prime}\times (0,T)$ $S\subset \bar{\mathcal B}_i$}. Then, there exists a constant $C$ \magenta{maybe $\sigma$, $s$ large enough} such that
%
...
&\quad+\left.\iint_{Q}e^{-2s\tilde{\alpha}_2}(s\tilde{\xi}_2)^{-1}|-\frac{1}{\ell^2}\varphi\csin{\mathcal B_1}+\frac{1}{\gamma^2}\varphi \csin{\mathcal B_2}|^2\dx\dt+\iint_{\Sigma^{\prime}}e^{-2s\tilde{\alpha}_2}\left|\frac{\partial \theta}{\partial n}\right|^2\dx\dt\right)\\ \label{igualda1}
...
Note that we can replace the functions $\tilde{\xi}_i, \tilde{\alpha}_i$ by $\beta_i, \phi_i$ in the interval $[T/2,T]$ in the second integral of $I(q,i,m)$ hence,

\begin{equation}
\int_{T/2}^{T}\int_{\Omega}e^{-2s\beta_i}(s\phi_i)^{3+m}|q|^2\dx\dt\leq I(q,i,m).
\end{equation}
%
...
\begin{align}\notag
&\int_{T/2}^{T}\int_{\Omega}e^{-2s\beta_1}\phi_1^{3}|\varphi|^2\dx\dt+\int_{T/2}^{T}\int_{\Omega}e^{-2s\beta_2}\phi_2^{2}|\theta|^2\dx\dt\\ \label{aux00}
...
\begin{equation}
\label{aux1}
\int_{\Omega}|\varphi(0)|^2\dx\leq C\left(\int_0^{3T/4}\int_{\mathcal{O}_{d}}|\theta|^2\dx\dt+\int_{T/2}^{3T/4}\int_{\Omega}|\varphi|^2\dx\dt\right)
\end{equation}

Through standard arguments and adding the integral to the equation \eqref{aux1} we have the inequality

\begin{equation}
\label{aux2}
\begin{split}
\int_{\Omega}|\varphi(0)|^2\dx+&\int_0^{T/2}\int_{\Omega}|\varphi|^2\dx\dt+\int_0^{T/2}\int_{\Omega}|\theta|^2\dx\dt\\
&\leq C\left(\int_0^{T/2}\int_{\Omega}|\theta|^2\dx\dt+\int_{T/2}^{3T/4}\int_{\Omega}|\theta|^2\dx\dt+\int_{T/2}^{3T/4}\int_{\Omega}|\varphi|^2\dx\dt\right). 
\end{split}
\end{equation}

Using classic energy estimates we will absorbe the term $\int_0^{T/2}\int_{\Omega}|\theta|^2\dx\dt$ in \eqref{aux2}. First, we apply Green's identities to the equation 

\begin{equation*}\int_{\Omega}\left(\theta\theta_t-\theta\Delta \theta\right)\dx=\int_{\Omega}\left(-\frac{1}{\ell^2}\varphi\csin{\mathcal B_1}+\frac{1}{\gamma^2}\varphi \csin{\mathcal B_2}\right)\theta\dx,\end{equation*}
%
and we use the equality $\theta\theta_t=\frac{1}{2}\frac{d}{dt}|\theta|^2$ to get,

\begin{align}\label{aux3}
\frac{1}{2}\frac{d}{dt}\int_{\Omega}|\theta|^2\dx+\int_{\Omega}|\triangledown \theta|^2\dx&=\int_{\Omega}\left(-\frac{1}{\ell^2}\varphi\csin{\mathcal B_1}+\frac{1}{\gamma^2}\varphi \csin{\mathcal B_2}\right)\theta\dx, 
\end{align}
then applying H\"older and young's inequalities we get,

\begin{align}\label{aux5}
\frac{1}{2}\frac{d}{dt}\int_{\Omega}|\theta|^2\dx+\int_{\Omega}|\triangledown \theta|^2\dx&\leq C\left(\frac{1}{\ell^4}\int_{\mathcal B_1}|\varphi|^2\dx+\frac{1}{\gamma^4}\int_{\mathcal B_1}|\varphi|^2\dx\right)+\frac12 \int_{\Omega}|\theta|^2\dx
\end{align}

Applying Gronwall's inequality in \eqref{aux3}, integrating in $[0,T/2]$ and using that \magenta{$ \mathcal {B}_1, \mathcal {B}_2\subset\Omega$} we have,

\begin{align}\label{aux4}
\int_{0}^{T/2}\int_{\Omega}|\theta|^2\dx\dt\leq C\frac{1}{\mu}\int_{0}^{T/2}\int_{\Omega}|\varphi|^2\dx\dt,
\end{align}
%
where $\mu=\min\{\ell^4,\gamma^4\}$. Hence from \eqref{aux2}, taking $\mu$ large enough and the fact that $\beta_i$ and $\phi_i$ are constant on $[0,T/2]$ and they have lower bound on $[T/2,3T/4]$, we obtain 

\begin{equation}
\label{aux6}
\begin{split}
\int_{\Omega}|\varphi(0)|^2\dx+&\int_0^{T/2}\int_{\Omega}e^{-2s\beta_1}\phi_1^{3}|\varphi|^2\dx\dt+\int_0^{T/2}\int_{\Omega}e^{-2s\beta_2}\phi_2^{2}|\theta|^2\dx\dt\\
&\leq C\left(\int_{T/2}^{T}\int_{\Omega}e^{-2s\beta_2}\phi_2^{2}|\theta|^2+e^{-2s\beta_1}\phi_1^3|\varphi|^2\dx\dt\right),
\end{split}
\end{equation}
%
in consequence, combining the inequalities \eqref{aux00} and \eqref{aux6} we get

\begin{equation}
\label{aux7}
\begin{split}
\int_{\Omega}|\varphi(0)|^2\dx+&\int_0^{T}\int_{\Omega}e^{-2s\beta_1}\phi_1^{3}|\varphi|^2\dx\dt+\int_0^{T}\int_{\Omega}e^{-2s\beta_2}\phi_2^{2}|\theta|^2\dx\dt\\
&\leq C\left(\iint_{\Sigma^{\prime}}e^{-2s\beta_1}\phi_1\left|\frac{\partial \varphi}{\partial n}\right|^2\dx\dt+\iint_{\Sigma^{\prime}}e^{-2s\beta_2}\left|\frac{\partial \theta}{\partial n}\right|^2\dx\dt\right).
\end{split}
\end{equation}

Now, observe that from properties given in Notation \ref{notation3.1} we can define $\hat{\beta}_2:=\max_{x\in S^{'}}\beta_2$. Then proceeding as before, applying Green's identity and using $\frac12\frac{d}{dt}e^{-2s\hat{\beta}_2}|\theta|^2=e^{-2s\hat{\beta}_2}\theta \theta_t-se^{-2s\hat{\beta}_2}|\theta|^2(\hat{\beta}_2)_t$ to equation  
%
\begin{equation*}\int_{\Omega}\left(\theta e^{-s\hat{\beta}_2}\theta_t-\theta e^{-s\hat{\beta}_2}\Delta \theta\right)\dx=\int_{\Omega}\left(-\frac{1}{\ell^2}\varphi\csin{\mathcal B_1}+\frac{1}{\gamma^2}\varphi \csin{\mathcal B_2}\right)e^{-s\hat{\beta}_2}\theta\dx,\end{equation*}
%
and using H\"older and young's inequalities we get

\begin{equation}
\label{aux8}
\begin{split}
\frac12\frac{d}{dt}\int_{\Omega}e^{-2s\hat{\beta}_2}|\theta|^2\dx+&\int_{\Omega}e^{-2s\beta_2}|\triangledown \theta|^2\dx\\
&\leq \frac{C}{\ell^4}\int_{\mathcal B_1}|\varphi|e^{-2s\hat{\beta}_2}\dx+\frac{C}{\gamma^4}\int_{\mathcal B_2}|\varphi|e^{-2s\hat{\beta}_2}\dx+\frac12\int_{\Omega}e^{-2s\hat{\beta}_2}|\theta|^2\dx.
\end{split}
\end{equation}

Applying Gronwall's inequality in \eqref{aux8} and integrating in $[0,T]$ imply

\begin{equation}
\label{aux9}
\int_0^T\int_{\Omega}e^{-2s\hat{\beta}_2}|\theta|^2\dx\dt\leq \frac{C}{\ell^4}\int_0^T\int_{\mathcal B_1}|\varphi|^2e^{-2s\hat{\beta}_2}\dx\dt+\frac{C}{\gamma^4}\int_0^T\int_{\mathcal B_2}|\varphi|^2e^{-2s\hat{\beta}_2}\dx\dt.
\end{equation}

We can apply the same analysis to de equation

\begin{equation*}\int_{\Omega}\left(\bigtriangleup\theta e^{-s\hat{\beta}_2}\theta_t-\bigtriangleup\theta e^{-s\hat{\beta}_2}\Delta \theta\right)\dx=\int_{\Omega}\left(-\frac{1}{\ell^2}\varphi\csin{\mathcal B_1}+\frac{1}{\gamma^2}\varphi \csin{\mathcal B_2}\right)e^{-s\hat{\beta}_2}\bigtriangleup\theta\dx,\end{equation*}
%
 to get 
%
\begin{equation}
\label{aux10}
\int_0^T\int_{\Omega}e^{-2s\hat{\beta}_2}|\triangle\theta|^2\dx\dt\leq \frac{C}{\ell^4}\int_0^T\int_{\mathcal B_1}|\varphi|^2e^{-2s\hat{\beta}_2}\dx\dt+\frac{C}{\gamma^4}\int_0^T\int_{\mathcal B_2}|\varphi|^2e^{-2s\hat{\beta}_2}\dx\dt.
\end{equation}
%
therefore \eqref{aux9} and \eqref{aux10} yields 

\begin{equation}
\label{aux11}
\iint_{Q}e^{-2s\hat{\beta}_2}|\theta|^2\dx\dt+\iint_{Q}e^{-2s\hat{\beta}_2}|\triangle\theta|^2\dx\dt\leq \frac{C}{\ell^4}\int_0^T\int_{\mathcal B_1}|\varphi|^2e^{-2s\hat{\beta}_2}\dx\dt+\frac{C}{\gamma^4}\int_0^T\int_{\mathcal B_2}|\varphi|^2e^{-2s\hat{\beta}_2}\dx\dt.
\end{equation} 

Finally, substituting \eqref{aux11} in the following trace estimate 

\begin{equation*}
\iint_{\Sigma^{'}}\left|\frac{\partial \theta}{\partial n}\right|^2e^{-2s\hat{\beta}_2}\dx\dt\leq C\left(\iint_{Q}e^{-2s\hat{\beta}_2}|\theta|^2\dx\dt+\iint_{Q}e^{-2s\hat{\beta}_2}|\bigtriangleup\theta|^2\dx\dt\right),
\end{equation*}
%
implies that 
% 
\begin{equation}
\label{aux12}
\iint_{\Sigma^{'}}\left|\frac{\partial \theta}{\partial n}\right|^2e^{-2s\hat{\beta}_2}\dx\dt\leq C \left(\frac{1}{\ell^4}\int_0^T\int_{\mathcal B_1}|\varphi|^2e^{-2s\hat{\beta}_2}\dx\dt+\frac{1}{\gamma^4}\int_0^T\int_{\mathcal B_2}|\varphi|^2e^{-2s\hat{\beta}_2}\dx\dt\right),\end{equation}
and the result is obtained applying \eqref{aux12} in \eqref{aux7}.


\magenta{ya no me fije si realmente se obtiene la eq 3.7}
