...
\author{ V\'ictor Hern\'andez-Santamar\'ia \thanks{E-mail: \texttt{vhernandez@ctrl.cinvestav.mx}} \and Liliana Peralta \thanks{E-mail: \texttt{liliana\_peralta@uaeh.edu.mx}}}


\maketitle

\abstract{In this paper, we present some controllability results for parabolic equations in the framework of hierarchic control. In the first part, we present a Stackelberg strategy combining the concept of controllability with robustness: the main control (the leader) is in charge of a null-controllability objective while a secondary control (the follower) solves a robust control problem, this is, we look for an optimal control in the presence of the worst disturbance. We improve previous results by considering that either the leader or follower control acts on a small part of the boundary.  
...
%
Optimization and control problems arise in many applications of engineering and mathematics. Traditionally, such problems deal with a single objective: minimize cost, maximize benefit, etc. When studying more realistic and complex situations, it is desirable to include several different objectives.  Therefore, the introduction of multi-objective optimization is essential. 
...
In the framework of control of PDEs, the so-called hierarchic control was introduced in \cite{LionsHier,LionsSta} by J.-L. Lions to study a bi-objective control problem for the heat equation. Such methodology uses the notion of Stackelberg optimization (\cite{Stackelber}) and deals with a non-cooperative decision problem where one of the participants (the \emph{leader}) enforces its strategy on the other participants (the \emph{followers}). In what follows, we recall briefly the notion of hierarchic control.

 Let $\Omega\subset \mathbb{R}^N$, $N\geq 1$, be a bounded open set with boundary $\partial \Omega$ regular enough and let $\omega$ and $\mathcal O$ be nonempty subsets of $\Omega$, with $\omega\cap\mathcal O=\emptyset$. Given $T>0$, we denote the cylinder domain $Q=\Omega\times(0,T)$ with lateral boundary $\Sigma=\partial \Omega\times(0,T)$. 

Let us consider the controlled heat equation
%
\begin{equation}\label{heat_lin}
\begin{cases}
y_t-\Delta y=h\chi_{\omega}+v\chi_{\mathcal O}, & \text{in Q}, \\
y=0 &\text{on } \Sigma, \\
...
In \eqref{heat_lin}, $y=y(x,t)$ is the state, $y^0$ is the initial datum, and $h=h(x,t)$ and $v=v(x,t)$ are control functions applied on $\omega$ and $\mathcal O$, respectively. Here, $\chi_A$ denotes the characteristic function of the set $A$. 

We would like to choose $v$ and $h$ in order to achieve two different optimal objectives:
%
\begin{enumerate}
\item[1)] find the minimal $L^2$-norm control $h$ such that $y(\cdot,T)=0$,
\item[2)] find the best control $v$ such that $y$ is ``not too far from'' a desired target $y_d$. 
\end{enumerate}
%

\magenta{REESCRIBIR:
These objectives can be stated more precisely by introducing the cost functionals
%
\begin{equation}
J_1(h)=\frac{1}{2}\iint_{\omega\times(0,T)}|h|^2\dx\dt
\end{equation}
%
and
%
\begin{equation}
J_2(v)=\frac{1}{2}\iint_{\mathcal O_d\times(0,T)}|y-y_d|^2\dx\dt+\frac{\beta}{2}\iint_{\mathcal O\times(0,T)}|v|^2\dx\dt,
\end{equation}
where $\beta>0$ is a constant, $ \mathcal O_d\subset \Omega$ is a non empty open subset and $y_d\in L^2(\mathcal O_d\times (0,T) )$ is given. In this way, the first and second optimal objectives become in: find controls $h$ and $v$ such that
%
blablabla...
}

Problem \ref{} is a null-controllability  
...
As above, we can associate a cost functional for the first objective and design the control $v$ subject to the null control controllability constraint. On the other hand, consider the quadratic cost functional
%
\begin{equation}\label{func_clasico}
J_2(v)=\frac{1}{2}\iint_{\mathcal O_d\times (0,T)}|y-y_d|^2+\frac{\beta}{2}\iint_{\omega\times(0,T)}|v|^2,
\end{equation}
%
where $\beta>0$ is a constant, $ \mathcal O_d\subset \Omega$ is a non empty open subset and $y_d\in L^2(\mathcal O_d\times (0,T) )$ is given. Then, the second control objective is to achieve  the optimization problem
%
\begin{equation}\label{min_clasico}
\min_{v\in L^2(\omega\times(0,T)) } J_2(v).
\end{equation}
%
This classical optimal control problem has been thoroughly studied, see, for instance, \cite{Lions_optim,trol}, and the references within. Since the functional $J_2$ is continuous, strictly convex and coercive, there exists a unique element $\bar v\in L^2(\omega\times(0,T))$ such that \eqref{min_clasico} holds. 

It is clear that the control problems 1-2 are well posed and have a solution. Nevertheless, it is not clear how to design the control $v$ satisfying both control objectives. In fact,  it is impossible in general to  choose the same $v$ fulfilling the control problems 1-2.

According to the original work of Lions, to apply this idea we divide first the control set $\omega$ into two parts
%
\begin{equation*}
\omega=\omega_1\cup\omega_2, \quad\text{up to a set of measure $\omega$},
\end{equation*}
%
where $\omega_i$ are open sets of $\Omega$ and $\omega_1\cap\omega_2=\emptyset$. If $\chi_{\omega_i}$ denotes the characteristic function of $\omega_i$ and $v^{i}$ denotes the restriction of $v$ to $\omega_i\times(0,T)$, then \eqref{heat_lin} becomes
%
\begin{equation}\label{heat_hier}
\begin{cases}
y_t-\Delta y=v^1\chi_{\omega_1}+v^2\chi_{\omega_2}, \quad \text{in Q}, \\
y=0 \quad\text{on } \Sigma, \\
y(0)=y^0, \quad \text{in } \Omega.
\end{cases}
\end{equation}
%
For given $v^i\in L^2(\omega_i\times(0,T))$, $i=1,2$, system \eqref{heat_hier} admits a unique solution that we denote as $y(v^1,v^2)=y(x,t;v^1,v^2)$. 

Following \cite{LionsSta}, we will design first the follower control $v^2$. To this end, consider the problem 
%
\begin{equation}\label{heat_prob_foll}
\left\{
\begin{aligned}
&\text{given $v^1\in L^2(\omega_1\times(0,T))$, find} \\
&\min_{v^2} J_2(v^1,v^2)
\end{aligned}
\right.
\end{equation}
%
where 
%
\begin{equation}\label{func_clasico_sta}
J_2(v^1,v^2)=\frac{1}{2}\iint_{\mathcal O_d\times (0,T)}|y-y_d|^2+\frac{\beta}{2}\iint_{\omega_2\times(0,T)}|v^2|^2, 
\end{equation}
%
and $y$ is the solution to \eqref{heat_hier}. Problem \eqref{heat_prob_foll} admits a unique solution for each $v^1\in L^2(\omega_1\times(0,T))$. Indeed, note that $v^1$ participates in the minimization problem by means of the variable $y$, but it can be seen as a constant during the process. Therefore, we write the solution as
%
\begin{equation}\label{v2_sol}
v^2=\mathcal F(v^1).
\end{equation}
%
In the second step, we replace $v^2$ as given in \eqref{v2_sol} and obtain
%
\begin{equation}\label{sol_id}
y(v^1,\mathcal F(v^1)). 
\end{equation}
%
Then, in a very natural fashion, we address the problem
%
\begin{equation}\label{heat_prob_lid}
\left\{
\begin{aligned}
&\text{find $v^1\in L^2(\omega_1\times(0,T))$, which minimizes} \\
&J_1(v^1)=\frac{1}{2}\iint_{\omega_1\times(0,T)}|v^1|^2 \quad\text{subject to}\quad y(\cdot,T;v^1,\mathcal F(v^1))=0. 
\end{aligned}
\right.
\end{equation}
%
...
\begin{itemize}
\item \textit{Step 1.} Given $v^1$ (the leader), we choose $v^2$ (the follower) using \eqref{heat_prob_foll}--\eqref{v2_sol}.
\item \textit{Step 2.} We choose the leader $v^1$ using \eqref{heat_prob_lid}.
\end{itemize}
}
%
...
%
\begin{equation}\label{heat_b1}
\begin{cases}
y_t-\Delta y=h\mathbf{1}_{\omega}+\psi \quad &\text{in $Q$}, \\
y=v\mathbf{1}_{\mathcal O} \quad &\text{on $\Sigma$,} \\
...
and
%
\begin{equation}\label{heat_b2}
\begin{cases}
z_t-\Delta z=v\mathbf{1}_{\mathcal B_1}+\psi\mathbf{1}_{\mathcal B_2} \quad &\text{in $Q$}, \\
z=h\mathbf{1}_{\mathcal S} \quad &\text{on $\Sigma$,} \\
z(x,0)=z_0(x) \quad &\text{in $\Omega$}.
\end{cases}
\end{equation}
where $h=h(x,t)$ and $v=v(x,t)$ are control functions, $\psi=\psi(x,t)$ is an undesired perturbation, and  $y_0,z_0$ are given initial data. All of these functions are given in appropriate functional spaces and will be clarified below.

Here, we want to choose the control functions  $v$ and $h$ in such way that the following optimality problems are solved: 
%
\begin{itemize}
...
\end{itemize}
%
The first problem, originally introduced in \cite{temam_nonlinear}, looks for a control such that a given cost functional achieves its minimum in presence of the worst disturbance case. In this way, the controlled system will be robust to other possible perturbations. The second problem is a classical null controllability problem and it has been studied for a broad class of systems described by parabolic PDEs, see for instance \cite{cara_guerrero} and the references within. 
...
Let us consider a nonempty open set $\mathcal O_d\subset \Omega$. We define the cost functional
%
\begin{equation}\label{func_rob}
J_r(v,\psi;h)=\frac{1}{2}\iint_{\mathcal O_d\times(0,T)}|y-y_d|^2dxdt+\frac{1}{2}\left[\ell^2\iint_{\Gamma\times(0,T)}|v|^2d\sigma dt-\gamma^2\iint_Q|\psi|^2dxdt\right]
\end{equation}
%
where $\ell,\gamma>0$ are constants and $y_d\in L^2(\mathcal O_d\times(0,T))$ is given. This functional describes the robust control problem. We look for an optimal pair such that simulatenously maximize $J_r$ with respect to $\psi$ and minimize it with respect to $v$, while maintaining the state $y$ ``close enough'' to a desired target $y_d$ in the observation domain $\mathcal O_d\times(0,T)$. The problem will be solved when a saddle point $(\bar v,\bar \psi)$ is reached (see Definition \ref{defi_rob} below). 
...
%
\begin{enumerate}
\item For a fixed control $h$, we look for an optimal pair $(\bar v,\bar \psi)$ solving the robust control problem:
%
\begin{definition}\label{defi_rob}
Let $h\in L^2(\omega\times(0,T))$ be fixed. The control $\bar v\in L^2(\Gamma\times(0,T))$, the disturbance $\bar \psi\in L^2(Q)$ and the state $\bar y=\bar y(h,\bar v,\bar \psi,y_0)$ solution to \eqref{heat_b1} associated to $(\bar v,\bar \psi)$ are said to solve the robust control problem when a saddle point $(\bar v,\bar \psi)$ (which depends on $h$) of the cost functional $J_r$ is achieved, namely
%
\begin{equation}\label{saddle}
J_r(\bar v,\psi;h)\leq J_r(\bar v,\bar \psi;h)\leq J_r(v,\bar \psi;h), \quad \forall (v,\psi)\in L^2(\Gamma\times(0,T))\times L^2(Q).
\end{equation}
%
In this case,
%
\begin{equation}\label{saddle_minmax}
J_r(\bar v,\bar \psi;h)=\max_{\psi\in L^2(Q)}\min_{v\in L^2(\Gamma\times(0,T))}J_r(v,\psi;h)=\min_{v\in L^2(\Gamma\times(0,T))}\max_{\psi\in L^2(Q)}J_r(v,\psi;h).
\end{equation}
%
\end{definition}
%
...
Aqui van los resultados (que a\'un no tengo...), algunas hipotesis utiles  

\begin{equation}\label{cond_omega}
\omega\cap\mathcal O_d\neq \emptyset
\end{equation}
...
%
\bibitem{zuazua_ondas}
\textsc{E. Zuazua.}
\newblock Exact boundary controllability for the semilinear wave equation.
\newblock {\em Nonlinear partial differential equations and their applications,} Vol. X (Paris 1987--1988), 357--391, Pitman Res. Notes Math. Ser., 220, Longman Sci. Tech., Harlow, 1991. 
...
\bibitem{bodart_fabre}
\textsc{O. Bodart and C. Fabre.}
\newblock Controls insensitizing the norm of the solutions of a semilinear heat equation.
\newblock {\em J. Math. Anal. and App.}, \textbf{195}, 1995, 658--683.
%
\bibitem{b_gb_r_1}
\textsc{O. Bodart, M. Gonz\'alez-Burgos and R. P\'erez-Garc\'ia.}
\newblock Insensitizing controls for a semilinear heat equation with a superlinear nonlinearity. 
\newblock {\em C.R. Acad. Sci. Paris, Ser I}, \textbf{335}, 8 (2002), 677--682.
%
\bibitem{b_gb_r_2}
\textsc{O. Bodart, M. Gonz\'alez-Burgos, and R. P\'erez-Garc\'ia.}
\newblock Insensitizing controls for a heat equation with a nonlinear term involving the state and the gradient. 
...
%
\bibitem{boyer_ell}
\textsc{F. Boyer, F. Hubert, and J. Le Rousseau.}
\newblock Discrete Carleman estimates for elliptic operators and uniform controllability of semi-discretized parabolic equations.
\newblock {\em J. Math Pures Appl.,} \textbf{93} (2010), 240--276.
%
\bibitem{boyer_ell_N}
\textsc{F. Boyer, F. Hubert, and J. Le Rousseau.}
\newblock Discrete Carleman estimates for elliptic operators in arbitrary dimension and applications.
\newblock {\em SIAM J. Control Optim.,} \textbf{48} (2010), 5357--5397.
%
\bibitem{boyer_full}
\textsc{F. Boyer, F. Hubert, and J. Le Rousseau.}
\newblock Uniform null-controllability for space/time-discretized parabolic equations.
\newblock {\em Numer. Math.,} \textbf{118} (2011), 601--661.
%
\bibitem{boyer_par}
\textsc{F. Boyer and J. Le Rousseau.}
\newblock Carleman estimates for semi-discrete parabolic operators and application to the controllability of semi-linear and semi-discrete parabolic equations.
\newblock {\em Ann. I. H. Poincar\'e--AN,} \textbf{31} (2014), 1035--1078.
%
%
\bibitem{mamadou_guerrero}
\textsc{N. Carre\~no, M. Gueye, and S. Guerrero.}
\newblock Insensitizing control with two vanishing components for the three-dimensional Boussinesq system. 
\newblock {\em to appear in ESAIM Control Optim. Calc. Var.}
%
%
...
%
\bibitem{kavian_deT}
\textsc{O. Kavian and L. de Teresa.}
\newblock Unique continuation principle for systems of parabolic equations.
\newblock {\em ESAIM Control Optim. Calc. Var.}, \textbf{16}, 2 (2010), 247--274.
%
\bibitem{labbe}
\textsc{S. Labb\'e and E. Tr\'elat.}
\newblock Uniform controllability of semidiscrete approximations of parabolic control systems.
\newblock {\em Syst. Control Lett.}, \textbf{55} (2006), 597--609.
%
  
%%
\bibitem{lions}
\textsc{J.-L. Lions.}
\newblock Quelques notions dans l'analyse et le contr\^{o}le de syst\`{e}mes \`a donn\'{e}es incompl\`{e}tes.
\newblock {\em Proceedings of the XIth Congress on Differential Equations and Applications/First Congress on Applied Mathematics,} University of M\'{a}laga, 1990, 43--54.
%
%
