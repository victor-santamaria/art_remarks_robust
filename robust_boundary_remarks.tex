...
%
\begin{theorem}\label{teo_main1} \magenta {no entiendo este resultado, cuales son las hip\'otesis y cuales las concluiones?}
...
Suppose that $\psi\equiv 0$. Then, there exist $\ell_0$ and a positive function $\rho=\rho(t)$ blowing up at $t=T$ such that for any $\ell>\ell_0$, $w_0\in L^2(\Omega)$ and $y_d\in L^2(\mathcal O_d\times(0,T))$ \magenta{faltan algo no entiendo la redacci\'on de los teoremas}
\begin{equation*}%\label{integ_yd}
\iint_{\mathcal O_d\times(0,T)}\rho^2|y_d|^2dxdt<+\infty,
\end{equation*}
