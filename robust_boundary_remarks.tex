...
Observe that the first statement implies Theorem \ref{teo_main1}. The equivalence between the statements of Proposition \ref{prop_control} is by now standard and relies on several classical arguments, so we omit the proof. For the interested reader, we refer to \cite{araruna,vhs_deT_rob} where such equivalence was addressed for similar hierarchic control problems. 

Hence, our task amounts to prove \eqref{obs_ineq_1}. To do so, let us introduce first the Hilbert space
%
\begin{equation*}
...
\begin{proposition}
Assume that $\varphi^T\in H_0^1(\Omega)$ and that $\ell,\gamma$ are large enough. Then, \eqref{adj_sys_foll} admits a unique solution $(\varphi,\theta)\in H^{2,1}(Q)\times W(Q)$.  
\end{proposition}
%
\begin{proof}
...
\end{equation}
%
Due to the regularity of $\varphi$, from \cite[Th. 2.1, p.9]{lions_magenes} we have that 
%
\begin{equation}
\frac{\partial\varphi}{\partial n}\in H^{\frac12,\frac14}(\Sigma)
\end{equation}
%
and, moreover, since $\varphi \mapsto \frac{\partial \varphi }{\partial n}$ is a continuous and linear map of $H^{\frac12,\frac14}(\Sigma)\to H^{2,1}(Q)$ we have
%
...
On the other hand, for $\varphi$ solution to \eqref{heat_fixed}, we consider the system
%
...
Now, consider the mapping defined by $\Lambda\bar\theta=\theta$ where $(\varphi,\theta)$ is the solution to the cascade system \eqref{heat_fixed},\eqref{theta_fixed}. From the linearity of the systems, estimate \eqref{est_theta_fixed} and Aubin-Lions Theorem (see, e.g., \cite{simon}) we see that $\Lambda$ is a continous and linear map of $L^2(Q)\to L^2(Q)$. 

For any $\bar\theta_1,\bar\theta_2\in L^2(Q)$, we consider the difference $\Lambda\bar\theta_1-\Lambda\bar\theta_2$. Then, it is not difficult to see that
%
\begin{equation}
\|\Lambda \bar\theta_1-\Lambda \bar\theta_2\|
\end{equation}
%
\end{proof}
...
\end{equation}
%
with $f\in L^2(Q)$, $u_0\in L^2(\Omega)$ and $g\in H^{\frac12,\frac14}(\Sigma)$ where we have denoted the Hilbert space $H^{r,s}(\Sigma)=L^2(0,T;H^{r}(\partial \Omega))\cap H^{s}(0,T;L^2(\partial \Omega))$, for $r,s\geq 0$, and equipped with the norm
%
\begin{equation}
\|u\|_{H^{r,s}(\Sigma)}:=\left(\|u\|_{L^2(0,T;H^r(\partial \Omega))}^2+\|u\|^2_{H^s(0,T;L^2(\partial \Omega))}\right)^{1/2}.
\end{equation}
%
...
%
 ...
 Our task now is to prove the observability inequality \eqref{obs_ineq_1}. It will be consequence of global Carleman inequalities and some energy estimates
