...
It is not difficult to prove that for any fixed $h\in L^2(\Sigma)$ the exists a unique saddle point $(\bar v,\bar \psi)\in L^2(\Sigma)\times L^2(Q)$ for the cost functional \eqref{func_rob}. Indeed, the procedure is practically the same as in Section \ref{ex_uniq_saddle} since the leader control $h$ is fixed and participates in a indirect way. The characterization of the saddle point then leads to the system
%
\begin{equation*}%\label{sys_sec3}
\begin{cases}
w_t-\Delta w=\frac{1}{\gamma^2}r, &\quad  \text{in Q}, \\
-r_t-\Delta r= (w-w_d)\mathbf{1}_{\mathcal O_d}  &\quad  \text{in Q}\\ 
w=h\mathbf{1}_{\mathcal O_1}+ \frac{1}{\ell^2}\frac{\partial r}{\partial n}\chi_{\mathcal O_2}, \quad r=0 &\quad \text{on } \Sigma, \\
w(x,0)=w^0(x),\quad r(x,T)=0 &\quad \text{in } \Omega.
\end{cases}
\end{equation*}
%

As in Section \ref{sec_bound_leader}, the control $h$ of minimal norm can be obtained by solving a suitable minimization problem. This would require to prove an observability of the form
%
\begin{equation}
...
We could opt to apply the Carleman inequality presented in Lemma \ref{lemma_car_boundary} fo the first equation of \eqref{adj_sys_3} while the Carleman estimate \eqref{car_boundary} to the second equation in \eqref{adj_sys_3}. Nevertheless, the definition of their respective weights are based on the selection of the functions $\eta_i$ (see eq. \eqref{constr_bound}) and $\eta^0$ (see eq. \eqref{constr_bound}). The different nature of such functions complicates the absorption of the lower order terms when adding both estimates and, moreover, as a consequence of applying \eqref{car_boundary} to the equation verified by $\theta$, we would have local term depending $\theta$ for which it is not clear how to eliminate. 

As mentioned in Section \ref{sec_intro}, we shall present a hierarchic control result for the case when $\psi\equiv 0$ and the optimal control objective is modified as follows. Let us choose any function $\rho_\star(t)\geq e^{\frac{s\alpha}{2}}$ and consider the cost functional
%
\begin{equation}\label{func_teo3}
\mathcal I(v;h)=\frac{1}{2}\iint_{\mathcal O_d\times(0,T)}|w-w_d|^2\dx\dt+\frac{\ell^2}{2}\iint_{\Sigma}\rho_\star^2|v|^2{\csbd}_1\d\sigma\dt
\end{equation}
%
